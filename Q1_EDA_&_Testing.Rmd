...
both %>%
  ...
  geom_boxplot() + labs(title = "Energy of Top 50 Songs by Country",
              caption = "Figure 1.2: Boxplot of Energy by Country")+
    scale_fill_manual(values = c("#6699cc", "#cd5c5c")) +
  xlab("Country") + ylab("Energy")+ 
  theme(plot.title = element_text(hjust = 0.5), plot.caption = element_text(hjust = 0.5))
```
<br />
In Figure 1.2, we can see there is a significant difference between the countries. The median energy is higher for the UK than the USA. The interquartile range is also much larger for the UK than the USA.

```{r}
# Boxplot for valence variable
both %>%
  ggplot( aes(x = country, y = valence, fill=country)) +
  geom_boxplot() + labs(title = "Valence of Top 50 Songs by Country",
              caption = "Figure 1.3: Boxplot of Valence by Country")+
    scale_fill_manual(values = c("#6699cc", "#cd5c5c")) +
  xlab("Country") + ylab("Valence")+ 
  theme(plot.title = element_text(hjust = 0.5), plot.caption = element_text(hjust = 0.5))
```
<br />
In Figure 1.3, we can see there is some difference between the countries. The median valence is higher for the UK than the USA. The interquartile range is also higher for the UK than the USA.
...
```
<br />
In Figure 1.4, we can see there is not much difference between the countries. The mean danceability is slightly higher for the UK (0.64) than the USA (0.63).

```{r}
# Histogram for danceability variable, overlaid, add to appendix?
ggplot(both, aes(x = danceability, fill = country)) +
...
```
<br />
In Figure 1.5, we can see there is a difference between the countries. The mean energy is higher for the UK (0.62) than the USA (0.54).

```{r}
# Histogram for energy variable, overlaid, add to appendix?
ggplot(both, aes(x = energy, fill = country)) +
  geom_histogram(position = "identity", alpha = 0.5, binwidth = .1)+
    scale_fill_manual(values = c("#6699cc", "#cd5c5c"))+
...
```
<br />
In Figure 1.6, we can see there is a difference between the countries. The mean valence is higher for the UK (0.5) than the USA (0.4).

```{r}
# Histogram for valence variable, overlaid, add to appendix?
ggplot(both, aes(x = valence, fill = country)) +
  geom_histogram(position = "identity", alpha = 0.5, binwidth = .1)+
    scale_fill_manual(values = c("#6699cc", "#cd5c5c"))+
